...
import { updateDailyMachine } from "@/actions/daily-machines"
import { useToast } from "@/hooks/use-toast"
import { Edit2, Check, X } from "lucide-react"

interface EditDailyMachineAmountProps {
  dailyMachineId: string
  currentAmount: number | null
}

export function EditDailyMachineAmount({
  dailyMachineId,
  currentAmount,
}: EditDailyMachineAmountProps) {
  ...
  const handleSave = async () => {
    ...
    const result = await updateDailyMachine(dailyMachineId, { amount })
    if (result.success) {
